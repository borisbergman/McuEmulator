...
class McuInterpret(threading.Thread):
    def mcuInterpreter(self):  #interpret command and reply
        ...
        print("received: {0} {1} {2} ".format(self.ReceivedCorrect, str(self.ThreadID),
                                              str(ByteToHex(self.ReceivedData))))
        ...
        #MCU Update
            ...
            #4.1.4 DSP Save Load
                ...
                self.Eeprom[address * 4:address * 4 + amount * 4] = [struct.unpack('B', x)[0] for x in
                                                                     self.ReceivedData[8:8 + amount * 4]]
            ...
            vuReturnCounter += 1
            command = [0x12, 0x06]
            for q in [x for x in struct.pack('h', int(math.sin(vuReturnCounter) * 1000))]:
                command.append(q)
            ...
            if self.ReceivedData[4] == bytes.fromhex('01'):
                ...
                self.generateCommand(
                    [0x12, 0x0B, 0x21, 0x00, 0x00, 0x00, 0x42, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02,
                     0x00])
                ...
                self.generateCommand([0x12, 0x01])
            elif self.ReceivedData[4] == bytes.fromhex('0F'):
                print("keep alive")
                self.generateCommand([0x12, 0x01])
            elif self.ReceivedData[4] == bytes.fromhex('0B'):
                self.generateCommand(
                    [0x12, 0x10, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0x01, 0x01, 0x01, 0x01])
            else:
                print("unknown")
        else:
    ...
    def run(self):
        ...
        while (1):
            ...
            if self.ReceivedData[len(self.ReceivedData) - 1] != self.getBytesChecksum(self.ReceivedData):
                print("incorrect checksum:" + str(self.ReceivedData[len(self.ReceivedData) - 1]) + "!=" + str(
                    self.getBytesChecksum(self.ReceivedData)))
                continue
